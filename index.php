...
$host = $matches[1];
$path=substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], 'index.php')+9);
$result=shell_exec('curl '.$destinationURL.$path);
echo $result;
//$domainName = $host;
//$ch = curl_init();
//curl_setopt($ch, CURLOPT_URL ,$destinationURL.$path);
...
/*if($domainName != $RequestDomain) {
    ...
    $response = proxy_request($destinationURL.$path, ($method == "GET" ? $_GET : $_POST), $method);
    $headerArray = explode("\r\n", $response[header]);
